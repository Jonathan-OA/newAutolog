                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <div class="row buttons_grid">
                            <a class="btn btn-success"  href="{!! route('allowedTransfers.create') !!}">@lang('buttons.add')</a>
                        </div>
                        <div class="panel-body">
                            @include('allowed_transfers.table')
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    $(function() {
        ...
        table = $("#allowedTransfers-table").DataTable({
            scrollX: true,
            scrollY: "47vh",
            ajax: 'allowedTransfers/datatable',
            autoWidth: true,
            fixedColumns:   {
                leftColumns: 0,
                rightColumns: 1
      ...
      $('#allowedTransfers-table tbody').on( 'click', 'button', function () {
            var data = table.row( $(this).parents('tr') ).data();
            var id = $(this).attr('id');
            if(id == 'edit'){
                //Editar Registro
                window.location.href = "{!! URL::to('allowedTransfers/"+data.id+"/edit') !!}";
            }else{
                //Excluir Registro
