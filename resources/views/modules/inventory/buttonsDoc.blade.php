<script type="text/ng-template" id="options">    
    <div id="hhhaaa" class="ui-grid-cell-contents" style="overflow: visible !important;"  ng-controller="MainCtrl">
        ...
        <span ng-if="row.entity.document_status_id == 0">
            ...
            <button ng-click="callRoute('./document/liberate/'+row.entity.id+'/inv')" class="icon_action" aria-label="@lang('buttons.liberate')" data-microtip-position="left" role="tooltip">
                <img class='icon' src='{{ asset('/icons/liberar.png') }}'> 
            </button>
             <!-- Editar (Apenas status 0) -->
             <button ng-click="callRoute('./inventory/'+row.entity.id+'/edit')" class="icon_action" aria-label="@lang('buttons.edit')" data-microtip-position="left" role="tooltip">
        ...
        <button ng-if="row.entity.document_status_id == 1 || row.entity.document_status_id == 2 "
                ng-click="callRoute('./document/return/'+row.entity.id+'/inv')"
