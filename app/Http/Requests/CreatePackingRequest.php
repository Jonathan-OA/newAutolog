...
class CreatePackingRequest extends FormRequest
{
    ...
    {
        return [ 
            'level' => 'required|integer|unique:packings,level,NULL,id,company_id,'.Auth::user()->company_id.',product_code,'.Input::get('product_code').'',
            'uom_code' => 'required|string|exists:uoms,code|max:6|unique:packings,uom_code,NULL,id,company_id,'.Auth::user()->company_id.',product_code,'.Input::get('product_code').'',
            'barcode' => 'required|string|unique:packings,barcode,NULL,id,company_id,'.Auth::user()->company_id.'|max:40',
